...
const FIELDS = [ 'sell', 'return', 'DT thuần', 'Tổng DT', 'Tổng vốn', 'Lợi nhuận' ];
const NUM_ROWS_STEP = 20;
const CELL_HEIGHT = 50;
const HEAD_HEIGHT = 60;
const borderColor = '#e7e6e1';
const WIDTH = [170, 110, 90, 120, 90, 90, 90, 90, 110, 90, 110, 90, 90, 110, 140, 130, 130, 130];
...
class Report extends React.Component {
    ...
    getData() {
        ...
        reportdetail_api({mtype: 'reportOrderDetail', depot_id, datef, datet}).then(({listOrders}) => {
            this.setState({ data: listOrders, loading: false });
        });
    }
        ...
        if (value === 'sell') {
            ...
            return (
                <View key={value} style={[styles.cell, {width}, is_header && {height: HEAD_HEIGHT}]}>
                    <Text style={[is_header && styles.white, {textAlign: 'center'}]}>{value}</Text>
                </View>
            );
        }
    }
    ...
    formatHeader() {
        ...
        return (
            <View style={styles.header}>
                {this.formatCell('Mã hóa đơn', WIDTH[0], true)}
    ...
    formatIdentityColumn(data) {
        const invoice_column = [this.formatCell('[1]',WIDTH[0]), this.formatCell(null, WIDTH[0])];
        const date_column = [this.formatCell('[2]',WIDTH[1]), this.formatCell('Tổng', WIDTH[1])];
        
        data.forEach((v, k) => {
            if (k > 1) {
                invoice_column.push(this.formatCell(v.invoice, WIDTH[0]));
                date_column.push(this.formatCell(v.date, WIDTH[1]));
            }
        });
        return (
            <View style={{flexDirection: 'row'}}>
                <View style={{width: WIDTH[0]}}>{invoice_column}</View>
    ...
    formatBody(data) {
        ...
        return (
            <View style={{ flexDirection: 'row'}}>
                {this.formatIdentityColumn(data)}
    ...
    onChange(value) {
        ...
        //if (parseInt(value) === 1) this.props.navigation.navigate('SellDetailLineChart', { datef, datet, depot_id });
    }

    render() {
        const { data, date, refreshing, visibleModal, loading, datef, datet, depot_id } = this.state;
        const db = [];
        !isEmpty(data) && db.push(this.formatBody([...data]));

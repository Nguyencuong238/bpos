...
const WIDTH = [180, 110, 120, 120, 120, 120, 120, 120, 130];
...
class Report extends React.Component {
    ...
    getData() {
        ...
        reportdetail_api({mtype: 'reportCashFlow', depot_id, datef, datet, is_mobile: 1}).then(({listOrders}) => {
        ...
        return (
            <View style={styles.header}>
                <View style={[styles.cell, {width: WIDTH[0], height: HEAD_HEIGHT}]}>
                    <Text style={styles.white}>Mã hóa đơn</Text>
                </View>
    ...
    onChange(value) {
        ...
        //if (parseInt(value) === 1) this.props.navigation.navigate('ReceiptsDetailLineChart', { datef, datet, depot_id });
    }

    render() {
        const { data, refreshing, visibleModal, loading, datef, datet, depot_id } = this.state;
        const db = [];
        if(!isEmpty(data)) {
            db.push(this.formatBody([...data]));
        }
        return (

...
class LineReport extends React.Component {
    ...
    render() {
        ...
        if (!isEmpty(data)) {
            data.forEach( v => {
                const tong_tm = parseInt(v.thu_tm) - parseInt(v.chi_tm);
                const tong_ck =  parseInt(v.thu_ck) - parseInt(v.chi_ck);
                const p = tong_tm + tong_ck;
                const d = v.date;
                date.push(p);
                profit.push(d);
            });
                                ...
                                <View style={styles.container}>
                                    ...
                                    <View style={{ flex: 1,  }}>
                                        ...
                                        <XAxis
                                            style={{ marginTop: 10 }}
                                            data={date}
                                            formatLabel={(index) => index % 3 ? null : moment(date[index], 'DD/MM/YYYY').format('DD')}

...
class ReportProductByUserId extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
            ...
            productSearch: null,
            tableHead: [ 'Tên hàng','SL nhân viên', 'SL mua', 'Doanh thu', 'SL trả','Giá trị trả','Doanh thu thuần'],
    ...
    renderItem() {
        ...
        let dtt = []
        forEach(data, (v, k) => {
            let total4 = v.count_personnel;
            let total5 = v.hoadon;
            let total6 = v.price_doanhthu;
            let total7 = v.trahang;
            let total8 = v.price_trahang;
            let total9 = v.price_doanhthu-v.price_trahang;
            const tmp = [v.name, v.count_personnel, v.hoadon,v.price_doanhthu,v.trahang,v.price_trahang,v.price_doanhthu-v.price_trahang];
            rowData.push(tmp);
            count_personnel.push(total4);
        ...
        const test2 = [['',sum(count_personnel), sum(hoadon), sum(price_doanhthu),sum(trahang),sum(price_trahang),sum(dtt)],];
        return (
            <View style={styles.table}>
...
export default connect(mapStateToProps)(ReportProductByUserId);

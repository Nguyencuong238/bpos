...
class ReportProductByManufacturer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
            ...
            productSearch: null,
            tableHead: [ 'Tên hàng','SL NCC', 'SL nhập', 'Giá trị nhập', 'SL trả','Giá trị trả','Doanh thu thuần'],
    ...
    renderItem() {
        ...
        let dtt = []
        forEach(data, (v, k) => {
            let total4 = v.count_supplier;
            let total5 = v.phieunhap;
            let total6 = v.giatrinhap;
            let total7 = v.trahangnhap;
            let total8 = v.giatritra;
            let total9 = v.giatrinhap-v.giatritra;
            const tmp = [v.name, v.count_supplier, v.phieunhap,v.giatrinhap,v.trahangnhap,v.giatritra,v.giatrinhap-v.giatritra];
            rowData.push(tmp);
            count_supplier.push(total4);
            phieunhap.push(parseInt(total5));
            giatrinhap.push(parseInt(total6));
            trahangnhap.push(parseInt(total7));
            giatritra.push(parseInt(total8));
            dtt.push(total9);
            
        })
        tableData.push(rowData);
        const test = [['[3]', '[4]', '[5]', '[6]', '[7]','[8]','[9=6-8]'],];
        const test2 = [['',sum(count_supplier), sum(phieunhap), sum(giatrinhap),sum(trahangnhap),sum(giatritra),sum(dtt)],];
        return (
            <View style={styles.table}>
...
export default connect(mapStateToProps)(ReportProductByManufacturer);
